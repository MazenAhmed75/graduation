...
class InvestmentService {
  ...
  DocumentReference _fundRef(String uid) => _db.collection('users').doc(uid);
  ...
  Stream<List<InvestmentModel>> getPortfolioStream(String uid) {
    return _portfolioRef(uid).snapshots().map(
      (snap) => snap.docs
          .map(
            (d) =>
                InvestmentModel.fromMap(d.id, d.data() as Map<String, dynamic>),
          )
          .toList(),
    );
  }
    ...
    return _tradesRef(uid)
        ...
        .map(
          (snap) => snap.docs
              .map(
                (d) =>
                    TradeModel.fromMap(d.id, d.data() as Map<String, dynamic>),
              )
              .toList(),
        );
  }
  ...
  Future<String?> executeBuy({
    ...
    if (posSnap.exists) {
      ...
      final existing = InvestmentModel.fromMap(
        posSnap.id,
        posSnap.data() as Map<String, dynamic>,
      );
      final newQty = existing.quantity + quantity;
      final newAvgPx =
          ((existing.quantity * existing.buyPrice) + usdAmount) / newQty;
      batch.update(posRef, {
        'quantity': newQty,
        'buyPrice': newAvgPx,
  ...
  Future<String?> executeSell({
    ...
    final position = InvestmentModel.fromMap(
      posSnap.id,
      posSnap.data() as Map<String, dynamic>,
    );
  ...
  Future<void> refreshPortfolioPrices(
    String uid,
    List<MarketAssetModel> liveAssets,
  ) async {
    final batch = _db.batch();
    ...
    final lines = positions
        .map(
          (p) =>
              ...
              '(${p.pnlPercent.toStringAsFixed(1)}%)',
        )
        .join('\n');
    return 'Portfolio:\n$lines\nAvailable cash: \$${fundBalance.toStringAsFixed(2)}';
  }
